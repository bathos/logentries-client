...
function Logger(opts) {
	...
	opts = {
		...
		flattenArrays: opts.flattenArrays === false ? false : true,
		host:          opts.host || defaults.host,
		levels:        opts.levels,
		minLevel:      opts.minLevel,
		secure:        opts.secure, 
		port:          opts.secure ? defaults.portSecure : defaults.port,
		replacer:      _.isFunction(opts.replacer) && opts.replacer,
		token:         opts.token,
	...
	var baseReplacer = lu.baseReplacer(opts.withStack);

	var replacer = _.flow(opts.replacer || lu.passReplacer, baseReplacer);

	var serialize = _.partial(jsonStringifySafe, _, replacer);

	defs._stringify = {
		value: opts.flatten ? lu.flat(serialize, opts.flattenArrays) : serialize
	};
	...
	log: {
		...
		value: function(lvl, log) {
			...
			if (_.isUndefined(log) || _.isNull(log))
				return this.emit('error', new Error(msg.noLogMessage));
			...
			if (!lvlName)
				return this.emit(
					'error',
					new Error(msg.unknownLevel({ lvl: lvl }))
				);
